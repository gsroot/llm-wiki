...
| # | 저장소 | 패턴 | 발견 회차 |
|---|--------|------|------|
| 1 | anthropics/skills | 표준 정의 (SKILL.md 패키지) | 3회차 |
| 2 | github/spec-kit | 메소드론 어댑터 + Codex Skills 통합 | 7회차 |
| 3 | fastapi/fastapi | 라이브러리 self-hosted SKILL.md | 9회차 |
| 4 | astral-sh/uv | `CLAUDE.md = @AGENTS.md` import (1줄) | 10회차 |
| 5 | scikit-learn/scikit-learn | SLEP + AGENTS.md (965B) AI disclosure | 11회차 |
| 6 | flutter/flutter | vendor-neutral `.agents/` + agentskills.io | 12회차 |
| 7 | openai/openai-cookbook | AGENTS.md "Recent Learnings" 살아있는 운영노트 | 13회차 |
| **8** | **openai/openai-agents-python** | **AGENTS.md=CLAUDE.md 동기화 + 9개 운영 SOP 스킬** | **14회차 (현재)** |
...
[[openai-cookbook]] 13회차에서 발견된 `articles/codex_exec_plans.md`(16KB) 메소드론을 본 저장소가 **5,485B로 응축하여 자체 PLANS.md로 내장**. 4가지 NON-NEGOTIABLE 요건:
...
**Codex가 자체 라이브러리 개발 운영에 사용**되는 첫 명시 사례. Stop 훅은 작업 종료 시 자동으로 repo 정리(임시 파일 삭제 등). cookbook 13회차의 codex_exec_plans.md 메소드론이 본 저장소 운영에 박힌 두 번째 증거 (PLANS.md + Codex 훅).
...
cookbook 13회차에서 발견된 ExecPlan 메소드론을 본 저장소가 자체 운영에 풀스택 적용:
...
- **uv** ([[astral-sh-uv]] 10회차): `uv.lock` 831KB, `uv sync` / `uv run python` Makefile 명령
...
OpenAI 공식 SDK가 **Astral 도구 스택(uv + ruff)을 채택**한 첫 메인스트림 사례. fastapi(9회차) 디폴트 스택과 동일한 방향. **pyright를 mypy 대신 채택**한 점은 결정적 — Microsoft도 OpenAI도 동일한 정적 타입 검사기 표준에 수렴.
...
1. **`agents/` 서브디렉토리 9개 본체 수집** — 9개 스킬 각각의 sub-agent 프롬프트가 어떤 패턴인지 (별도 회차로 분리 가능, 약 30~50KB 추가)
...
5. **MCP 1.19 의존성 추적** — 본 저장소가 [[mcp]] 1.19를 디폴트 채택. mcp 문서 별도 회차로 1.19 변경점 수집
6. **examples/financial_research_agent/, customer_service/, research_bot/** — 도메인별 reference agent 사례 분석 (회사 BI에 차용 가능 도메인 식별)
7. **번역 시스템 (`docs/scripts/translation`)** — co-op-translator (microsoft-for-beginners 4회차)와 다른 번역 파이프라인 비교
8. **OpenAI Agents SDK ↔ [[claude-agent-sdk]] 비교 분석** — 같은 회차 한 쌍으로 종합 분석 (별도 synthesis 페이지 후보)
...
### 가설 2: PLANS.md ExecPlan을 분기 대형 분석에 적용 (cookbook 13회차 가설 보강)

13회차 cookbook에서 가설로 제시했던 PLANS.md 패턴이 OpenAI 자체 SDK에서 풀스택 적용된 증거(본 회차)로 신뢰도 상승. c2spf-analytics 분기 코호트 분석 1건에 ExecPlan 양식 적용 PoC. 단일 분석가 + LLM 협업으로 7시간+ 단일 작업 가능성 검증.
...
본 회차는 13회차 [[openai-openai-cookbook]]의 후속이자 한 쌍. cookbook이 "OpenAI는 살아있는 운영 노트 패턴 + PLANS.md 메소드론 도입"이라고 박았다면, 본 회차는 **그 정의자가 자기 핵심 SDK 운영에 풀스택 적용**한 직접 증거. 두 회차로 OpenAI의 거버넌스 통합이 명확해짐:
...
- agents-python의 9개 sub-agent prompts 단독 수집 (이번 회차 raw에서 의도적 제외)
