...
MateChat v1.0.0은 **출시 직전 QA 단계**이며, QA 완료 후 Google Play Store에 정식 출시할 예정이다. 백엔드 83개 API, Flutter 132개 Dart 파일, 9개 언어, FCM 푸시, IAP 검증, Sentry/Grafana/Loki, 보안 감사와 QA 결과가 요약되어 있다.

[[seokgeun-kim-profile-2026]]도 동일하게 "Android v1.0.0 출시 직전 수준"으로 기록한다. 한때 본 프로젝트 위키 일부 raw 문서(`synthesis/implementation-status.md`, `synthesis/launch-readiness.md`)가 "출시 완료 및 운영 모드"로 잘못 기록되어 있었으나, owner 자기보고로 정정됐다. 출시 후에는 Play Console·배포 태그·mate-chat 저장소의 최신 문서로 운영 상태를 추적한다.
...
3. **출시 상태는 출시 직전 QA 단계로 정합됐다.** 한때 프로젝트 위키 일부 raw 문서가 "출시 완료"로 잘못 기록되어 있었으나, owner 자기보고로 정정. 출시 후에는 Play Console과 배포 태그로 추적한다.
