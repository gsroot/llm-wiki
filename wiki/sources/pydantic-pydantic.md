...
> 9년차 ★27.6K Python 데이터 검증 라이브러리. **타입 힌트만으로 스키마 정의 + 자동 검증 + JSON Schema 생성** 패턴을 정착시켰고, [[fastapi]](9회차) / [[openai-agents-python]](14회차) 모두 디폴트 의존성으로 채택. V2(2023)는 ground-up rewrite로 **`pydantic-core` Rust 확장**으로 성능 5~50배 개선 + V1 빌트인 `from pydantic import v1`으로 점진적 마이그레이션 보장. **명시 버전 정책 ("V1→V2 같은 breaking change는 다시 없다")** + `llms.txt` 표준 채택 + Logfire 관측성 자매 제품 출시.
...
- **자매 제품**: **Pydantic Logfire** (관측성), **Pydantic AI** (LLM 에이전트, 18회차 수집 예정)
...
이는 [[scikit-learn]](11회차)의 19년 안정성 / [[openai-agents-python]](14회차)의 Public API Positional Compatibility / [[ruff]] Preview Mode와 같은 **"안정성 vs 진화" 양립** 패턴.
...
[[openai-agents-python]] (14회차) `docs/llms.txt`와 같은 패턴. **사용자 docs를 LLM 색인용 마크다운으로도 노출**. llmstxt.org 표준의 **두 번째 메이저 라이브러리 채택 사례** (앞 사례: openai-agents-python). FastAPI 같은 라이브러리가 향후 채택할 신호.
...
Logfire는 19회차 수집 후보 (운영/Observability 회차) — Prometheus/Grafana/Sentry와 같은 카테고리.
...
이는 [[openai-agents-python]] (14회차)가 mypy 대신 **pyright** 채택한 이유와 직결. Pydantic 모델은 pyright가 정확히 추론 → Agents SDK도 pyright 채택 → BigQuery 결과 모델 → c2spf-analytics API → 전체 type chain 일관성. 회사 BI 적용 시 핵심 포인트.
...
- [[fastapi]] (9회차) — Pydantic 없이는 동작 불가
- [[openai-agents-python]] (14회차) — `pydantic>=2.12` 디폴트 의존성
- [[scikit-learn]] (11회차) — 직접 의존은 아니나 모델 메타데이터에 사용 가능
- [[uv]] (10회차) — 본체는 Rust지만 Python 측 사용자가 모두 Pydantic 사용
...
이는 [[fastapi]](9회차)의 핵심 디자인 패턴 — Annotated를 거점으로 검증·DI·문서화·OpenAPI를 모두 통합. SQLAlchemy 2.x도 같은 패턴 채택 (15회차 수집 예정).
...
[[openai-agents-python]] (14회차) llms.txt에 이은 두 번째 메이저 사례. 라이브러리 docs 사이트는 이제 **사람 시각용 (MkDocs/Sphinx) + LLM 색인용 (llms.txt)** 두 축이 표준화 중. 본 위키도 `wiki/llms.txt` 신설 + index.md를 자동 생성하는 PoC 후보.
...
1. **`pydantic-core` Rust 코드 분석** — 검증 핫패스가 어떻게 분리됐는지 (별도 회차 후보)
2. **Pydantic Logfire 본체 수집** — 19회차 운영/Observability 회차에서 Prometheus/Grafana/Sentry와 함께
3. **Pydantic AI 본체 수집** — 18회차 LLM 에이전트 회차에서
...
본 회차 회고 — Pydantic은 위키에 1차 자료가 없는 **사실상 가장 영향력 큰 Python 라이브러리** 중 하나였음. FastAPI(9회차) / openai-agents-python(14회차)이 모두 의존하는데 정작 Pydantic 자체는 미수집. 본 회차로 보강. 다음 자매 제품 후보:

- **Pydantic AI** (18회차 LLM 회차) — Pydantic 진영의 LLM 에이전트 SDK
- **Pydantic Logfire** (19회차 운영 회차) — 관측성

이 두 회차 후 Pydantic 진영 3제품(Pydantic / Pydantic AI / Logfire)이 완성되어 Astral 3제품(uv·ruff·ty), OpenAI 2제품(cookbook·agents-python)과 같은 "회사 차원 도구 일체화" 패턴 비교 가능.
