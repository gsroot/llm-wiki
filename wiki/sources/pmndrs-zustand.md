...
3. **TanStack Query와의 보완관계**: 클라이언트 상태(Zustand) + 서버 상태(TanStack Query) 듀얼 채택이 React 진영 사실상 표준. → 종합 페이지의 핵심 매트릭스.
...
raw 수집. Redux 후속 진영 중 "providerless" 미니멀 노선 대표. Zustand + TanStack Query + shadcn-ui 트리오는 사실상 모던 React 진영의 사이드 프로젝트 1순위 스택.
