...
1. **개인 비서 AI 모바일 앱 — Flutter 채택 결정**: iOS+Android+Web 3 플랫폼을 단일 코드베이스로 처리. Material 3 + ColorScheme.fromSeed로 다크모드까지 자동. 백엔드는 [[backend-python-fastapi]] FastAPI, 프론트는 Flutter — 회사 BI([[c2spf-analytics]]) React 스택과 다른 선택지를 하나 더 확보.
