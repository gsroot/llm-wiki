...
1. **AGENTS.md 12단계 진화 — `$skill` 인덱싱**: Next.js AGENTS.md는 `$pr-status-triage`, `$flags`, `$dce-edge`, `$react-vendoring`, `$runtime-debug`, `$authoring-skills` 6개 스킬을 `$<name>` syntax로 참조. AGENTS.md가 거대 hub로 진화하고 세부 SOP는 `.agents/skills/<name>/SKILL.md`로 분리. Sentry/Grafana의 계층화·anti-fragmentation에 이은 새 변종.
...
3. **Anti-pattern 명시 + Secret redaction**: "Never re-run the same test", "Never print secret values", "treat env values as sensitive unless test-mode flags". Sentry의 anti-fragmentation에 이은 운영 보안 거버넌스 명문화.
...
raw 수집. AGENTS.md 12단계 진화의 양대 변종 (`$skill` 인덱싱 + HTML PR 마커)을 단일 OSS에서 발견. 프론트엔드 진영의 유일한 AGENTS.md 채택자(5/1)로서 양극화 가설(운영 60% vs 프론트 20%) 추가 보강.
