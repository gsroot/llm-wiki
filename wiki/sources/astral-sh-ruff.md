...
├── CLAUDE.md (12B) ★ `@AGENTS.md` (uv 10회차 패턴 = 회사 차원 표준)
├── BREAKING_CHANGES.md
├── CONTRIBUTING.md
├── pyproject.toml
...
같은 회사 [[uv]](10회차 수집)와 **완전히 동일한 패턴**. 즉 Astral은 자체 도구 3제품 (ruff·uv·ty) 모두에서 같은 컨벤션을 채택. `ty`는 같은 저장소에 동거하므로 자동 적용. 이는:
...
`uvx`는 [[uv]] (10회차)가 제공하는 일회성 실행 명령. `prek`는 pre-commit-conformat (또는 비슷한) 도구. AGENTS.md가 "task 종료/리베이스/리뷰 코멘트 응대/푸시 전" 모두에서 자동 호출 강제. **회사 도구 스택을 AGENTS.md에 박음** 패턴 — fastapi(9회차)와 openai-agents-python(14회차)에서도 본 동일 패턴.
...
ruff의 `CLAUDE.md = @AGENTS.md`는 uv(10회차)와 정확히 같은 패턴 — 즉 새 패턴이 아니라 **같은 회사의 또 다른 제품**. 이는 다음을 시사:
...
[[fastapi]] (9회차) SKILL.md가 명시한 디폴트 스택 (FastAPI/SQLModel/Asyncer/HTTPX/Typer/uv/Ruff/ty) 중 **마지막 3개가 모두 Astral 제품**. Astral의 비전:

- **uv** = pip + virtualenv + pyenv + pipx + poetry 통합 (10회차 발견)
- **ruff** = Black + Flake8 + isort + pydocstyle + pyupgrade + autoflake 통합 (본 회차)
- **ty** = mypy / pyright 차세대 (개발 중)

세 도구 모두 Rust + 기존 도구 N개 통합. 회사 BI 적용 시 "Python 도구 단일 진입점"으로 c2spf-analytics 마이그레이션 ROI 분석 후속 후보. 9회차 fastapi 디폴트 스택 → 10회차 uv 본체 → 11회차 scikit-learn 거버넌스 → 14회차 openai-agents-python ruff/uv/pyright → 본 회차 ruff 본체로 **Astral 도구 스택의 위키 내 가시성 완성**.

### Insight 3: AGENTS.md = 14항목 개발 가이드라인 = AI 명령

ruff AGENTS.md는 anthropics-skills(3회차) SKILL.md 패키지나 openai-agents-python(14회차) 9개 SOP와는 다른 형태:
...
mdtest 패턴은 SLEP(scikit-learn 11회차) doctest와 다음과 같이 다름:
...
c2spf-analytics 적용 시 ROI 추정: pylint/black 분리 → ruff 통합 시 CI 30~60s → 5s 이하 (10회차 uv 마이그레이션과 같이 도입하면 더 큼).
...
이는 [[scikit-learn]](11회차) SLEP의 점진적 진화 패턴, [[openai-agents-python]](14회차)의 Public API Positional Compatibility와 **같은 "안정성 vs 진화" 양립 패턴**의 또 다른 형태.
...
이는 **"진실 단일 소스 + 자동 동기화"** 패턴 — 코드(Rust)가 진실, 룰 정의를 변경하면 자동으로 docs / JSON Schema / CLI reference 모두 재생성. [[openai-cookbook]](13회차) `check_notebooks.py` registry.yaml 자동 검증과 같은 계열.

본 위키도 `wiki/index.md`를 frontmatter에서 자동 생성하는 PoC로 차용 가능. (13회차 cookbook의 registry.yaml에서도 같은 가설 제시했으나 ruff는 이미 운영 중이므로 신뢰도 ↑)
...
1. **ty 본체 단독 수집** — `crates/ty_*` Rust 코드 + `crates/ty_python_semantic/resources/mdtest/` 본체 (별도 회차)
...
8. **Charlie Marsh 인터뷰/블로그 분석** — Astral 비전 1차 자료 (10회차에서 일부 수집됨)

## 회사 BI 적용 가설

### 가설 1: c2spf-analytics ruff 도입 (10회차 uv와 한 묶음)

10회차에서 가설로 제시한 "c2spf-analytics uv 마이그레이션"과 한 묶음으로 ruff 도입. CI 파이프라인 변화:
...
본 회차는 [[uv]](10회차)의 자매 제품 수집. 같은 [[astral]] 회사의 두 번째 제품이 같은 컨벤션을 정확히 답습 — agent-skills 외부 채택 패턴이 "조직 외부 채택" → "조직 내 표준화"로 한 단계 진화. 다음 후보로 ty 본체 단독 수집 시 **3제품 일체화**가 위키에 박힐 것. 또한 본 회차 발견 "회사 차원 표준화" 패턴은 향후 회차에서 같은 회사 여러 제품 수집 시 (예: pmndrs 진영의 Zustand·Jotai·Recoil, Vercel 진영의 Next.js·SWR·AI SDK) 다시 검증할 가설.
