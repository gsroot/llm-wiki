...
루트 `CLAUDE.md`가 12바이트 — `@AGENTS.md`. 같은 회사 [[uv]]와 동일 패턴 → **agent-skills 외부 채택 8단계 진화의 9번째 변형 사례 = "회사 차원 표준화"**.
...
- [[uv]]: 자매 도구, 같은 `CLAUDE.md = @AGENTS.md` 패턴
- [[fastapi]]: SKILL.md 디폴트 스택에 포함

## 의사결정 컨텍스트 (raw 인용)

> "Astral의 Python 도구 3제품(uv·ruff·ty) 중 첫 번째이자 ★47K Rust 단일 바이너리로 Flake8/Black/isort/pydocstyle/pyupgrade/autoflake 등 7개 이상 도구를 10~100배 빠르게 통합하고, 같은 회사의 uv와 동일한 CLAUDE.md = @AGENTS.md 1줄 import 패턴을 채택."
...
- [[astral-sh-ruff]] — 본 저장소 1차 수집

## 메모

- ruff = 본 위키에서 [[uv]]와 한 묶음 — Astral 회사 차원 표준화 검증
- ty 본체 단독 수집은 (Rust 본체 + mdtest 본체)
