...
Prometheus는 SoundCloud(2012)에서 시작해 2016년 CNCF에 합류, 2018년 두 번째 졸업 프로젝트(Kubernetes 다음)가 된 시계열 메트릭 모니터링 시스템. **단일 서버 노드 자율 + HTTP pull 모델 + PromQL** 3대 특징으로 클라우드 네이티브 모니터링의 사실상 표준이 됐다. 19회차에서 발견된 7번째 AGENTS.md 채택 OSS이며, 그 형식이 **PR 리뷰 패턴 가이드**라는 점에서 다른 OSS와 차별화된다.
...
- [[prometheus-prometheus]] — README (229줄) + AGENTS.md (148줄, PR 패턴) — 19회차 7번째 AGENTS.md OSS.
...
- 16회차 ASF PMC와 19회차 CNCF는 모두 vendor-neutral 재단이지만 영역 다름: ASF=데이터/JVM, CNCF=클라우드 네이티브 인프라/런타임.
