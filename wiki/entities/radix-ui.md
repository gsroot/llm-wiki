...
본 페이지는 **stub** — Radix UI 단독 저장소가 raw 수집 대상이 아닌 상태에서 [[shadcn-ui]] / [[react]] 페이지가 참조하므로 정합성 stub으로 등록.
...
- [[shadcn-ui]] "Open Code" 거버넌스 모델의 기술 기반 — `npx shadcn add button` = Radix Primitive + Tailwind 클래스를 사용자 코드베이스에 복사
...
- stub 사유: 점검에서 `[[radix-ui]]` 깨진 링크 발견. shadcn-ui source 기반으로 1차 보강.
