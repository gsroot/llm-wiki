...
[[seokgeun-stack-guide]] Python 백엔드 5단의 검증 축. [[matechat]] backend + [[c2spf-analytics]] 후보. **타입 힌트 → 자동 검증 + JSON Schema** 패턴은 [[fastapi]]·[[openai-agents-python]]·[[pydantic-ai]]가 모두 디폴트 채택. **V1→V2 ground-up rewrite + 명시 버전 정책 + llms.txt 채택**은 [[llm-infra-meta-cluster]] 5축의 LLM 친화 거버넌스 사례. Rust 확장 패턴은 [[ruff]]·[[uv]]·[[polars]]와 같은 Rust-in-Python 트렌드.
