...
9년차 ★27.6K Python 데이터 검증 라이브러리. **타입 힌트만으로 스키마 정의 + 자동 검증 + JSON Schema 생성** 패턴을 정착시킨 사실상 표준 — [[fastapi]] / [[openai-agents-python]] 모두 디폴트 의존성. V2(2023) ground-up rewrite로 `pydantic-core` Rust 확장 분리 + 5~50배 성능 개선 + V1 빌트인 `from pydantic import v1`로 점진적 마이그레이션 보장. 자매 제품 — Pydantic Logfire (관측성) / **[[pydantic-ai]]**.
...
README 첫 줄 배지에 `llms.txt-green`. [[openai-agents-python]]에 이은 **두 번째 메이저 라이브러리 채택 사례**.
...
> "9년차 ★27.6K Python 데이터 검증 라이브러리. 타입 힌트만으로 스키마 정의 + 자동 검증 + JSON Schema 생성 패턴을 정착시켰고, fastapi / openai-agents-python 모두 디폴트 의존성으로 채택. V2(2023)는 ground-up rewrite로 pydantic-core Rust 확장으로 성능 5~50배 개선."
> — [[pydantic-pydantic]] 한줄 요약

[[seokgeun-stack-guide|석근 32 OSS 스택 카탈로그]] Python 백엔드 5단의 검증 축. [[matechat|MateChat 사이드 프로젝트]] backend + [[c2spf-analytics|c2spf 게임 데이터 BI]] 후보. **타입 힌트 → 자동 검증 + JSON Schema** 패턴은 [[fastapi]]·[[openai-agents-python]]·[[pydantic-ai]]가 모두 디폴트 채택. **V1→V2 ground-up rewrite + 명시 버전 정책 + llms.txt 채택**은 [[llm-infra-meta-cluster|LLM 인프라 메타 5축]] 5축의 LLM 친화 거버넌스 사례. Rust 확장 패턴은 [[ruff]]·[[uv]]·[[polars]]와 같은 Rust-in-Python 트렌드.

## 출처

- [[pydantic-pydantic]] — 본 저장소 1차 수집

## 메모

- Pydantic은 위키에 1차 자료가 없는 **사실상 가장 영향력 큰 Python 라이브러리** — FastAPI / openai-agents-python 모두 의존
