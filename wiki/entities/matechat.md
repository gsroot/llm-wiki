...
> AI 소셜 메시징 (v1.0.0 출시 직전 QA 단계, QA 완료 후 Google Play 정식 출시 예정 — 44회차 owner 자기보고로 정정). 39 SKILL.md 운영 SOP (자작 11 + 외부 28). FastAPI + Flutter + Riverpod + OpenAI + WebSocket. 회사 BI([[c2spf-analytics]])와 "기술 스택 검증의 쌍"으로 동작. 자작 SKILL 9개가 c2spf 차용 후보. 출시 상태는 `verification_required: true` (33회차).
