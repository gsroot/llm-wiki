...
| 모델 # | 모델 | 대표 OSS | |
|---|---|---|---|
| 1~7 | Vendor PaaS, ASF PMC, BDFL 등 | (위키 누적) | |
| 8 | ASF PMC | Apache Kafka, Airflow | |
| 9 | CNCF graduated | Prometheus, Kubernetes | |
| **10** | **Open Code (코드 분배)** | **shadcn-ui/ui** | **** |
...
[[seokgeun-stack-guide|석근 32 OSS 스택 카탈로그]] React UI 컴포넌트 표준. [[c2spf-analytics|c2spf 게임 데이터 BI]] React 리뉴얼(2025) 컴포넌트 후보 + [[matechat|MateChat 사이드 프로젝트]] 사이드는 shadcn-ui-flutter 변형 채택. **Open Code 패러다임**(코드 분배 vs npm 분배)은 [[llm-infra-meta-cluster|LLM 인프라 메타 5축]] 5축의 OSS 거버넌스 카탈로그 10번째 모델로 등록 — [[bdfl]]·NumFOCUS·메일링 리스트와 함께 다양성 사례. [[radix-ui]] + [[tailwindcss]] 위에 빌드. shadcn 단일 메인테이너.

## 출처

- [[shadcn-ui-ui]] — 본 OSS 저장소

## 메모

신규 등록. MUI/Chakra/Mantine과 다른 "라이브러리 미설치" 노선. 113K stars로 1위지만 단일 메인테이너 + Vercel 후원 패턴은 9번째 거버넌스 모델(CNCF 다수 메인테이너) 정반대 끝. Anti-fragmentation의 정반대 = "의도적 fragmentation".
