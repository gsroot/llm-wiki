...
[[seokgeun-stack-guide]] React UI 컴포넌트 표준. [[c2spf-analytics]] React 리뉴얼(2025) 컴포넌트 후보 + [[matechat]] 사이드는 shadcn-ui-flutter 변형 채택(28회차 발견). **Open Code 패러다임**(코드 분배 vs npm 분배)은 [[llm-infra-meta-cluster]] 5축의 OSS 거버넌스 카탈로그 10번째 모델로 등록 — [[bdfl]]·NumFOCUS·메일링 리스트와 함께 다양성 사례. [[radix-ui]] + [[tailwindcss]] 위에 빌드. shadcn 단일 메인테이너.
