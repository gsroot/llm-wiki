...
컴투스 그룹의 게임 플랫폼 자회사. 다수의 모바일/PC 게임에 공통 기능(인증, 결제, 데이터 분석, 블록체인, 광고 트래킹 등)을 SDK·플랫폼 형태로 제공한다. 김석근(석근)이 2017-05부터 현재까지 백엔드/풀스택으로 재직 중인 회사.

## 주요 특징

- **GitHub 조직**: [c2spf-gsroot](https://github.com/c2spf-gsroot) — 내가 회사에서 수행한 프로젝트 저장소를 모아 둔 GitHub org.
- **주요 서비스 라인**
 - **애널리틱스(BI)** — `c2spf-gsroot/analytics-*` 레포 군. 게임 로그 수집·분석·시각화. → [[c2spf-analytics|c2spf 게임 데이터 BI]]
...
- 본인의 회사 프로젝트 GitHub 활동은 `김석근 <sgkim@com2us.com>` author 기준 `c2spf-gsroot` 조직 내 누적 1,349커밋 (2026-06-12 시점).
