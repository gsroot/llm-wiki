...
[[seokgeun-stack-guide|석근 32 OSS 스택 카탈로그]] React 클라이언트 상태 표준. [[tanstack-query]](서버 상태)와 듀얼 채택이 22회차 결론. [[c2spf-analytics|c2spf 게임 데이터 BI]] React 리뉴얼(2025) 클라이언트 UI 상태 후보. [[matechat|MateChat 사이드 프로젝트]] Flutter 진영 동등 위치는 [[riverpod]]. **providerless 미니멀 API**로 boilerplate 최소화 — [[llm-infra-meta-cluster|LLM 인프라 메타 5축]] 5축의 minimalist 디자인 철학 사례 ([[redis]] MANIFESTO와 비슷한 opinionated 모델). [[poimandres]] collective 산하.
