...
### 3. 현재 상태 해석: 기술 완료와 시장 검증의 분리

프로젝트 위키는 MateChat을 출시 완료/운영 모드에 가깝게 기록한다. 반면 석근 프로필은 출시 직전 수준이며 사용자 검증이 부족하다고 기록한다.

이 차이는 모순이지만, 핵심 판단은 같다. **기술적으로는 많이 만들었고, 다음 병목은 사용자 검증이다.** 출시 완료 여부가 어느 쪽이든, 지금 물어야 할 질문은 "앱이 실제 사람 간 대화를 만들어내는가"다.
...
- 프로젝트 위키의 "v1.0.0 출시 완료"와 개인 프로필의 "출시 직전" 중 최신 운영 상태는 무엇인가?
...
- [[matechat]] — llm-wiki의 MateChat 프로젝트 엔티티
