...
MateChat은 **출시 직전 QA 단계**다. 한때 프로젝트 위키 일부 raw 문서가 "출시 완료/운영 모드"로 잘못 기록했으나, owner 자기보고로 정정. 두 source([[mate-chat-project-wiki-2026]], [[seokgeun-kim-profile-2026]]) 모두 이제 동일 표현(출시 직전 QA 단계)으로 일치한다.
...
- ~~프로젝트 위키의 "v1.0.0 출시 완료"와 개인 프로필의 "출시 직전" 중 최신 운영 상태는 무엇인가?~~ → owner 자기보고로 "출시 직전 QA 단계"로 정합 완료.
