---
title: Mate Chat 채팅 분석 모듈 — 7축 분석 + BigQuery 파이프라인
type: synthesis
...
`/Users/sgkim/Projects/` 트리에서 채팅·데이터 분석 관련 4개 형제 프로젝트 발견. 디렉토리 트리만 확인하고 raw 수집은 후속:
...
→ Mate Chat 본진의 채팅 분석은 **(가설) 본진 PostgreSQL → 형제 분석 백엔드 → BigQuery → 분석 Flutter** 흐름이 가능하나, 본 raw 미수집으로 **공식적 검증 안 됨**. 3 형제 프로젝트 raw 수집 후 본 종합 페이지 보강.
...
[Mate Chat PostgreSQL]      [Mate Chat 본진 운영 중 — v1.0.0]
       ...
       ↓
[GCS Parquet 스테이징]      [Apache Arrow + Parquet 표준]
       │
...
→ **위키 의 거의 모든 도구가 단일 분석 파이프라인 내에 매핑 가능**.
...
[[matechat|MateChat 사이드 프로젝트]] stub에서 다음과 같이 적었음:
...
- 본 페이지의 BigQuery 파이프라인은 **(아직) 가설 영역에 머무름** — 형제 프로젝트 raw 수집으로 실제 흐름 확정 필요

## 미래 작업

1. **raw 수집**: 3 형제 프로젝트(mate-katok-analysis-backend / -flutter / data-mate)의 README + pyproject.toml + CLAUDE.md / project-plan.md / notebooks/ 1차 자료 수집. 본 종합 페이지 (a)~(e) 단계의 실제 도구 검증 또는 가설 폐기.
2. **7축 SQL 템플릿화**: 1·2·4·5번 축의 SQL 뷰 템플릿을 [[seokgeun-stack-guide|석근 32 OSS 스택 카탈로그]]에 "데이터 분석 30분 부트스트랩" 섹션으로 추가.
3. **이탈 예측 (6번 축) 별도 종합**: [[lightgbm]] / [[scikit-learn]] 적용 + 회사 BI 직접 차용 시나리오 분리 — 후보.
...
신규:
- [[seokgeun-mate-chat|석근 MateChat 본진 raw]] — Mate Chat 본진 1차 수집

기존 통합:
- [[matechat-project-knowledge-map]] — Mate Chat 위키 스냅샷
- [[seokgeun-operating-profile-2026]] — 석근 운영 프로필 / 사업화 맥락
- [[matechat]] entity

## 메모

- **본 종합은 가설 영역**: 형제 프로젝트 raw 미수집으로, 분석 모듈의 실제 구현이 본 페이지 BigQuery 파이프라인 가설과 일치하는지 검증 안 됨. **후속 작업**으로 명시.
- stub의 "BigQuery 파이프라인" 추정은 메모리 컨텍스트 S442에서 유래한 것으로, 본 raw 검증 시도 후 **분석 모듈이 본진과 분리된 별도 프로젝트로 운영**된다는 실태 확인.
...
- 한국어 채팅 데이터 특성상 토크나이즈·임베딩 다국어 모델 (예: bge-m3, multilingual-e5) 검토 필요. 형제 프로젝트 수집에서 어떤 임베딩 모델 사용 중인지 확인.
