...
이 페이지가 에이전트 스택의 시간순 진화를 설명한다면, [[llm-infra-meta-cluster|LLM 인프라 메타 5축]]는 그 진화에서 반복 등장한 [[agent-skills]] / [[harness]] / [[mcp]] / [[claude-code]] 4개 메타 노드를 묶은 상위 지도다. RAG 질의에서 "내 위키의 숨은 5번째 축"이나 "LLM 협업 운영 패턴"을 물으면 [[llm-infra-meta-cluster|LLM 인프라 메타 5축]]를 먼저 본다.
...
| 축 | 대표 자료 | 들어온 시점 |
|----|----------|------------|
| Microsoft | for-beginners 5종 통합 (`generative-ai`/`ai-agents`/`ml`/`web-dev`/`data-science`) | 2026-04-27 |
| Anthropic | `anthropics/skills` + `anthropics/claude-cookbooks` 짝 | 2026-04-27 |
| Karpathy | `autoresearch` + `nanoGPT` + `nanochat` | 2026-04-27 |
...
## 스냅샷 이후 — 4축·5축·6축 확장 노트 (2026-04-27 / 04-28 후속 추가)

본 페이지는 원래 Microsoft·Anthropic·Karpathy 3축 비교로 작성됐다. 같은 날 후속 수집된 [[github-spec-kit]]·[[flutter-flutter]]·[[openai-openai-cookbook]]이 각자 새로운 축을 박아 **6축 진화**로 확장:
...
| **Google (Flutter)** | **자산 [[vendor-neutral]] 화 + 토큰 예산 4계층** | flutter/flutter `.agents/` + `docs/rules/` (1k/4k/10k/full) | **표준 채택, 위치는 자체 결정** (`.agents/skills` + `.claude/skills` 심볼릭 링크) |
| **OpenAI (cookbook + agents-python)** | **살아있는 운영 노트 + 7시간+ 단일 작업 living document + 9개 운영 SOP 스킬 + 거버넌스 자기 채택** | (가이드 단) openai-cookbook `AGENTS.md` Recent Learnings + `articles/codex_exec_plans.md` PLANS.md + registry.yaml; (본체 단) openai-agents-python `.agents/skills/` 9 SKILL.md + PLANS.md(5.5KB) + AGENTS.md=CLAUDE.md 동기화 + `.codex/hooks.json` 자동화 + `examples/agent_patterns/` 11종 reference 구현 (Anthropic 5 + OpenAI 6확장) | **정적 가이드의 동적 진화 + self-adoption** |
...
- **OpenAI cookbook**: 1~5번째 축이 **정적 가이드**(한번 쓰면 정형화된 표준 / SKILL.md / SLEP / Constitution / rules.md / .agents 자산)였다면, OpenAI는 **AGENTS.md를 살아있는 운영 노트로 운영**한다. `Recent Learnings` 섹션 6개 항목이 "현상 → 대응 → 이유" 형식으로 운영 중 발견을 즉시 누적. 또한 **PLANS.md / ExecPlans** 패턴은 단일 LLM 작업 7시간+를 가능케 하는 자기완결 living document — NON-NEGOTIABLE 5 요건(자기완결 / 살아있는 / 초보자 구현 / 관찰 가능한 동작 / 본문 용어 정의). 추가로 `registry.yaml` 3,180줄 + `authors.yaml` 583줄 + `check_notebooks.py` 검증 도구로 **콘텐츠 거버넌스 자동화** — SLEP/PDEP의 표준 변경 거버넌스와 다른 콘텐츠 변경 거버넌스의 자동화 형태.
- **OpenAI agents-python**: cookbook이 **메소드론 정의 단**(가이드)이라면, [[openai-openai-agents-python]]은 같은 회사가 **자기 핵심 SDK 본체에 동일 패턴을 풀스택 적용**한 **거버넌스 자기 채택 (self-adoption)**의 직접 증거. 채택 4중 풀스택: (1) `.agents/skills/` **9개 운영 SOP 스킬** — `code-change-verification` / `openai-knowledge` / `implementation-strategy` / `pr-draft-summary` / `runtime-behavior-probe`(13.4KB ★) / `docs-sync` / `examples-auto-run` / `final-release-review` / `test-coverage-improver` — 각자 trigger·skip·workflow 정책 명시 + **스킬 간 호출 (skill chaining)** 명시. (2) **PLANS.md (5.5KB)** — cookbook의 `articles/codex_exec_plans.md`(16KB)를 응축한 자기 SDK용 ExecPlan 정의. (3) **`AGENTS.md = CLAUDE.md` 동기화** — 12,900B 양쪽 byte-for-byte 동일 미러링 (uv `@AGENTS.md` import와 flutter `.agents/` 심볼릭 링크의 중간). (4) **`examples/agent_patterns/` 16개 .py — 11종 패턴 reference 구현** — Anthropic 5 패턴 + OpenAI 6확장 (Guardrails 3종 / Human-in-the-loop 3종 / Forced tool use). 1년 14 메이저 버전(v0.14.6, 약 26일에 1 버전)의 빠른 반복 + **Public API Positional Compatibility** 정책으로 dataclass 필드 순서를 호환성 계약으로 격상. → cookbook이 "OpenAI는 정적 가이드의 동적 진화" 가설이었다면, **메소드론 ↔ 본체의 한 묶음 거버넌스**로 가설을 일반화. agent-skills 외부 채택 8단계 진화의 8번째이자 첫 "9개 본격 운영 SOP" 사례.

### 결론 갱신

- 원래 3축 결론(보완 관계, 표준-구현 분리 깊이, CI-로컬 채널, 다국어·접근성, 객관성 위계)은 6축에서도 유효
...
- **6축 추가의 결정적 인사이트**: 정적 가이드 5 사례 vs 살아있는 운영 노트 1 사례의 갈림. 5 사례 모두 "한 번 쓰면 정형화된 표준"으로 변경 비용을 고정하지만, OpenAI는 **변경 비용 ≈ 0** (Recent Learnings에 한 줄 추가만으로 가이드 진화). 이는 LLM 협업 가이드의 시간 단위가 분기/연간 → 일/시간 단위로 가속됨을 시사 — Anthropic 진영도 같은 방향으로 따라올 가능성.
- **시간 기반 검증의 등장**: PLANS.md ExecPlan은 "7시간+ 단일 LLM 작업"이라는 **시간 기반 차별 지표**를 박았다. 이는 1~5번째 축의 검증 메커니즘(코드 리뷰 / SLEP 투표 / Constitution 채택률 / SKILL.md 자동 호출 트리거 정확도)과 본질적으로 다른 새로운 평가 차원.
- **회사 BI 도입 가이드 보강**: Flutter의 `docs/rules/` 4계층 패턴은 BI 도메인의 `CLAUDE_1k.md/4k.md/10k.md` 분할에 직접 차용 가능. 다중 AI 도구를 BI에 통합할 때 (Cursor + Copilot + Claude Code + ChatGPT) 룰 파일을 자동 매칭. **+ OpenAI registry.yaml 패턴**은 c2spf-cookbook 신설 시 콘텐츠 거버넌스의 직접적 청사진. **+ AGENTS.md "Recent Learnings"** 패턴을 본 위키 자체에 차용하면 누적 운영 메타 메모 자동 생성.
...
- [[github-spec-kit]] — Spec-Driven Development 메소드론 + Specify CLI + 9 슬래시 명령 + 30+ 에이전트 통합. agent-skills 표준 첫 외부 채택
- [[flutter-flutter]] — Google ★176K 11년차 OSS의 vendor-neutral `.agents/skills/` + `docs/rules/` 4계층 토큰 예산
- [[openai-openai-cookbook]] — OpenAI ★73K 4년차 cookbook의 `AGENTS.md` Recent Learnings 살아있는 운영 노트 + `articles/codex_exec_plans.md` PLANS.md / ExecPlans + `registry.yaml` 콘텐츠 거버넌스 자동화. **6번째 축 (정적 가이드 → 살아있는 운영 노트), 메소드론 정의 단**
- [[openai-openai-agents-python]] — OpenAI ★25K 1년차 멀티 에이전트 Python SDK(v0.14.6)의 `.agents/skills/` 9개 운영 SOP 스킬 + PLANS.md(5.5KB) + AGENTS.md=CLAUDE.md 동기화 + `.codex/hooks.json` Stop 훅 자동화 + `examples/agent_patterns/` 16개 .py (Anthropic 5 + OpenAI 6확장 = 11종 reference 구현). **6번째 축 보강 (메소드론 ↔ 본체 한 묶음 self-adoption), 본체 적용 단**
- [[openai-chatgpt-codex-guide]] — 송영옥의 한국어 OpenAI 실무서 13장. **3대 코딩 에이전트 보완재 모델** 명문화: [[github]] Copilot(IDE 인라인) / [[claude-code]](로컬 터미널 페어 프로그래밍) / [[codex|Codex]](클라우드 비동기 자율 PR 생성). 같은 개발자가 작업 단계별로 3개를 병행 — 대체 관계가 아닌 영역 분담. agent-stack의 "사용자 측" 통합 모델로 추가됨.
...
- [[github]] — GitHub 축(spec-kit)의 운영 주체
- [[google]] — Google 축(Flutter)의 운영 주체
- [[openai]] — OpenAI 축(cookbook)의 운영 주체
...
- [[spec-kit]] — GitHub 축의 reference 구현
- [[flutter]] — Google 축의 reference 구현
- [[openai-cookbook]] — OpenAI 축의 reference 구현 (★73K, 289 콘텐츠 / 115 저자)
