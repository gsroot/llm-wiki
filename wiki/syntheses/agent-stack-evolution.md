...
| **OpenAI (cookbook + agents-python)** | **살아있는 운영 노트 + 7시간+ 단일 작업 living document + 9개 운영 SOP 스킬 + 거버넌스 자기 채택** | (가이드 단) openai-cookbook `AGENTS.md` Recent Learnings + `articles/codex_exec_plans.md` PLANS.md + registry.yaml; (본체 단) openai-agents-python `.agents/skills/` 9 SKILL.md + PLANS.md(5.5KB) + AGENTS.md=CLAUDE.md 동기화 + `.codex/hooks.json` 자동화 + `examples/agent_patterns/` 11종 reference 구현 (Anthropic 5 + OpenAI 6확장) | **정적 가이드의 동적 진화 + self-adoption** (정의자가 자기 본체에 풀스택 적용한 첫 증거 — cookbook 13회차에서 메소드론 정의 → agents-python 14회차에서 본체에 같은 패턴 박음) |
...
- **회사 BI 도입 가이드 보강**: Flutter의 `docs/rules/` 4계층 패턴은 BI 도메인의 `CLAUDE_1k.md/4k.md/10k.md` 분할에 직접 차용 가능. 다중 AI 도구를 BI에 통합할 때 (Cursor + Copilot + Claude Code + ChatGPT) 룰 파일을 자동 매칭. **+ OpenAI registry.yaml 패턴**은 c2spf-cookbook 신설 시 콘텐츠 거버넌스의 직접적 청사진. **+ AGENTS.md "Recent Learnings"** 패턴을 본 위키 자체에 차용하면 13회차 누적 운영 메타 메모 자동 생성.
