...
- 페이지 제목(H1): 한국어 우선 (예: `# 트랜스포머 아키텍처`)
- 내부 링크: Obsidian 위키링크 `[[파일명]]` 사용
- 태그: 한국어 + 영어 병기 (예: `tags: [인공지능, artificial-intelligence]`)
...
1. `index.md`에 등록되었지만 실제 파일이 없는 항목 (깨진 링크)
2. 파일은 있지만 `index.md`에 누락된 페이지 (미등록 페이지)
