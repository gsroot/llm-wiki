...
- 태그 (적용 범위 4단계 — 31회차 명확화 / 59회차 옵션 A 재해석):
  - **59회차 정책 재해석 (옵션 A — canonical + tag-alias 분리)**: 31회차 "한·영 병기 의무"는 실측 결과 vocabulary 분기를 야기했음 (예: `agent` 46 vs `에이전트` 19로 같은 의미가 두 노드로 자라남). 56회차 audit synthesis + 57회차 평가 P1 결정에 따라 다음 정책으로 전환:
    - **tags에는 canonical 1개만** 박는다. 표기 변종은 **frontmatter `aliases:`** (Obsidian 표준 alias)로 검색 변종을 흡수.
    - canonical 결정 원칙: **현재 우세 표기를 채택**한다 (정정 마찰 최소화). 새 페이지·새 태그는 canonical 결정 시점에서 우세 표기를 채택.
    - 회귀 차단: `wiki-lint.py` check #14가 canonical-alias 매핑 위반 자동 보고.
  - **6쌍 canonical 결정 (59회차 P0-1)**:
...
### RAG 답변 정책 (51회차 신설 — Codex 권고 채택)
LLM 에이전트가 위키를 RAG 컨텍스트로 사용해 답변할 때 따라야 할 신뢰도·랭킹 규칙. 50회차 평가에서 외부(Codex)가 "`source_count`를 LLM이 순진하게 신뢰하면 정의 A(운영자 의미)와 정의 B(자동 측정)를 혼동한다"고 지적했고, 이를 51회차에 정책 차원에서 명문화한다.
...
3. **메타 페이지 인용 금지**: `rag_exclude: true` 페이지(`index.md`, `logs/*.md`, redirect alias)는 답변 근거로 인용하지 말 것. 43회차 운영 원칙 재확인.
4. **약자 풀이 인용**: hub 페이지 첫 단락의 약자 풀이를 답변에서 한 번 이상 그대로 인용해 사용자가 약자만 보고 추론 부담을 갖지 않게 한다 (46회차 약자 풀이 237건 인프라 활용).
